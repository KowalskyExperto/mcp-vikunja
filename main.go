...
func main() {
	...
	mcp.AddTool(server, &mcp.Tool{Name: "list_kanban_buckets", Description: "List all kanban columns (buckets) of a project view"}, handler.ListKanbanBuckets)
	mcp.AddTool(server, &mcp.Tool{Name: "create_kanban_bucket", Description: "Create a new kanban bucket on a project view"}, handler.CreateKanbanBucket)
	mcp.AddTool(server, &mcp.Tool{Name: "update_kanban_bucket", Description: "Update an existing kanban bucket"}, handler.UpdateKanbanBucket)
	mcp.AddTool(server, &mcp.Tool{Name: "delete_kanban_bucket", Description: "Delete an existing kanban bucket"}, handler.DeleteKanbanBucket)
	mcp.AddTool(server, &mcp.Tool{Name: "move_task_to_bucket", Description: "Move a task to a different kanban column"}, handler.MoveTaskToBucket)
	mcp.AddTool(server, &mcp.Tool{Name: "list_labels", Description: "List all labels available to the user"}, handler.ListLabels)
	mcp.AddTool(server, &mcp.Tool{Name: "get_label", Description: "Get a label by ID"}, handler.GetLabel)
	mcp.AddTool(server, &mcp.Tool{Name: "create_label", Description: "Create a new label"}, handler.CreateLabel)
	mcp.AddTool(server, &mcp.Tool{Name: "update_label", Description: "Update an existing label by ID"}, handler.UpdateLabel)
